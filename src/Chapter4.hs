...
instance Functor List where
  fmap _ Empty = Empty
  fmap f (Cons e t) = Cons (f e) (fmap f t)
...
concat' Empty                = Empty
concat' (Cons Empty r)       = concat' r
concat' (Cons (Cons e r) rl) = Cons e $ concat' $ Cons r rl
...
toList Leaf = Empty
toList (Node v l r) = Cons v $ concat' $ Cons (toList l) $ Cons (toList r) Empty
